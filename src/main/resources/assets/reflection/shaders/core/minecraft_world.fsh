...
// ShaderToy "Minecraft" by Reinder Nijhoff (CC BY-NC-SA 4.0)
// https://www.shadertoy.com/view/4ds3WS  —— Notch 的 JS Minecraft + iq 体素光追
// 移植到 MC 剑面：iChannel 噪声贴图 → 程序化 value noise；iTime → time uniform；
// 相机自带飞行路径（与玩家视角无关）；输出按 mask.r 裁成剑形。
...
uniform float time;
uniform sampler2D Sampler0;   // 方块图集（采 mask 决定剑形与可见区域）
uniform vec2 uvMin;           // mask sprite 在图集的 UV 下界
uniform vec2 uvMax;           // mask sprite 在图集的 UV 上界
...
// 程序化 value noise（替代原 iChannel0 贴图采样）
...
float mapTerrain( vec2 p ) {
	...
	float f;
    f  = 0.500*noise( p*0.5 );   // 原 textureLod(iChannel1, p*0.01) → 低频程序噪声
...
void main() {
    // 剑表面 mask 局部 UV → 场景"屏幕"坐标 [0,1]
	vec2 q = clamp((texCoord0 - uvMin) / (uvMax - uvMin), 0.0, 1.0);
    vec2 p = -1.0 + 2.0*q;
    float aspect = (uvMax.x - uvMin.x) / max(uvMax.y - uvMin.y, 1e-6);
    p.x *= aspect;

	float st = 2.0*iTime;   // 场景相机时间（原 mainImage 里的 time，自动飞行）
    // camera
